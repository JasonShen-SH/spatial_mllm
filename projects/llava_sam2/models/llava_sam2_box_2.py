...
#         self.norm1_cross = nn.LayerNorm(embed_dim)
#         self.self_attn = nn.MultiheadAttention(embed_dim, num_heads, batch_first=True)
#         self.norm1_self = nn.LayerNorm(embed_dim)
#         self.ffn = nn.Sequential(
#             nn.Linear(embed_dim, ff_dim),
#             nn.GELU(),
#             nn.Linear(ff_dim, embed_dim)
#         )
#         self.norm2_ffn = nn.LayerNorm(embed_dim)

#     def forward(self, query, key_value, attn_mask_cross=None, attn_mask_self=None):
#         cross_attn_output, _ = self.cross_attn(
#             query, key_value, key_value,
#             attn_mask=attn_mask_cross
#         )
#         x = self.norm1_cross(query + cross_attn_output)
#         self_attn_output, _ = self.self_attn(x, x, x, attn_mask=attn_mask_self)
#         x = self.norm1_self(x + self_attn_output)
#         ffn_output = self.ffn(x)
#         output = self.norm2_ffn(x + ffn_output)
#         return output

# class CrossAttentionStack(nn.Module):
#     def __init__(self, embed_dim, ff_dim, num_layers, num_heads=1):
#         super().__init__()
...
#             query = layer(query, key_value, attn_mask_cross, attn_mask_self)
#         return query
...
class CABlock(nn.Module):
    ...
    def forward(self, query, key_value, attn_mask):
        # pdb.set_trace()
        attn_output, _ = self.attn(query, key_value, key_value,attn_mask=attn_mask)
...
class VideoLLaVASAMModel(LisaModel):
    ...
    def _add_special_tokens(self):
        ...
        self.box_token_idx = self.tokenizer("<box_sep>", add_special_tokens=False).input_ids[0]
...
class VideoLLaVASAMModel_zero3(VideoLLaVASAMModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        # self.tokenizer.decode(output['predicted_tokens'][0])
        
        # if gt_bboxes_areas or gt_bboxes_expression is None:
        #     box_valid = False
        # else:
        #     box_valid = True
        ...
        del g_pixel_values, gt_bboxes_areas, gt_bboxes_expression, gt_bboxes_points, frames_per_batch, input_ids
