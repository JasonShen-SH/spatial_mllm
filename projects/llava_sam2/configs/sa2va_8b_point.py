...
from projects.llava_sam2.datasets import VideoReVOSDataset_box_point, VideoMeVISDataset_box_point, VideoRefYoutubeVOSDataset_box_point
...
from projects.llava_sam2.datasets import ReferSegmDataset_box_point
...
special_tokens = ['<sep>', '<no_target>', '<next>', '<single_img>', '<points>', '</points>', '<p>', '</p>', '<vp>', '</vp>']
...
# ReVOS需要手动下载, 并放置在video_datas下，因为下载数据包不全
...
video_revos_box_file = 'mask_to_box/revos_bbox.json' # home目录下,下同
...
video_refytvos_mask_file = 'refytvos_mask_dict.pkl'
video_refytvos_box_file = 'mask_to_box/ref_ytvos_bbox.json'

video_revos_dataset = dict(
    type=VideoReVOSDataset_box_point,
...
video_mevis_dataset = dict(
    type=VideoMeVISDataset_box_point,
...
video_refytvos_dataset = dict(
    type=VideoRefYoutubeVOSDataset_box_point,
...
refcoco_segm_dataset=dict(
    type=ReferSegmDataset_box_point,
    tokenizer=tokenizer,
    special_tokens=special_tokens,
    extra_image_processor=extra_image_processor,
