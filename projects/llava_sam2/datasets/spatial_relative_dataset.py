...
RELATIVE_BOX_QUESTIONS = [
    "What is the {position}{suffix} object to the {direction} of <box>{ref_coords}</box> in the video?",
    "Count {position} objects to the {direction} of <box>{ref_coords}</box>. Which object is it?",
    "Find the {position}{suffix} object {direction} of <box>{ref_coords}</box> in the video.",
    "Locate the {position}{suffix} item {direction} of <box>{ref_coords}</box> in the video.",
    "Locate the {position}{suffix} item {direction} from <box>{ref_coords}</box> in the video.",
    "Identify the {position}{suffix} object {direction} from <box>{ref_coords}</box>.",
    "Identify and locate the {position}{suffix} object {direction} from <box>{ref_coords}</box> in the video.",
    "Locate and identify the {position}{suffix} object {direction} from <box>{ref_coords}</box>.",
    "Track the {position}{suffix} object {direction} from <box>{ref_coords}</box>.",
    "Track and locate the {position}{suffix} object {direction} from <box>{ref_coords}</box>.",
    "Track and identify the {position}{suffix} object {direction} from <box>{ref_coords}</box>.",
]

RELATIVE_BOX_ANSWERS = [
    "It is <box>{coords}</box>, located {position}{suffix} {direction} of the reference.",
    "It is <box>{coords}</box>, {position}{suffix} {direction} of the reference.",
    
    "It's located at <box>{coords}</box> in the video.",
    "It's <box>{coords}</box>.",
    "Sure, it is <box>{coords}</box>.",
    
    "Sure, it is <box>{coords}</box> in the video.",
]
...
class VideoSpatialRelativeDataset_box(VideoReVOSDataset_box):
    ...
    def prepare_text(self, n_frames, expressions, num_image_tokens=256, ref_boxes=None, org_image_size=None):
        ...
        for i, exp in enumerate(expressions):
            ...
            question_template = random.choice(RELATIVE_BOX_QUESTIONS)
            question = question_template.format(position=position_text, suffix=suffix, direction=direction, ref_coords=normalized_box)
            questions.append(question)
            answer_template = random.choice(RELATIVE_BOX_ANSWERS)
    ...
    def dataset_map_fn(self, data_dict, select_k=5, org_image_size=None):
        ...
        if self.use_fast: # False
            text_dict = self.prepare_text(select_k, expressions, num_image_tokens=self.patch_token, n_fast_images=len(fast_video_frames), ref_boxes=ref_boxes, org_image_size=org_image_size)
        else:
            text_dict = self.prepare_text(select_k, expressions, ref_boxes=ref_boxes, num_image_tokens=self.patch_token, org_image_size=org_image_size)
