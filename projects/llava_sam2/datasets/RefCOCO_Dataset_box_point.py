...
from projects.glamm.datasets.utils.utils import SEG_QUESTIONS, ANSWER_LIST, BOX_QUESTIONS, BOX_ANSWER_LIST, BOX_POINT_QUESTIONS, BOX_POINT_ANSWER_LIST
...
class ReferSegmDataset_box_point(RefCocoDataset):
    ...
    def _parse_annotations(self, ann_info):
        ...
        for real_idx, idx in enumerate(index):
            ...
            for seg_idx, seg in enumerate(inst["mask"]):
                ...
                if seg_idx == 0:
                    ...
                    tmp = points[real_idx]
                    points[real_idx] = []
                    points[real_idx].append(tmp)
                    points[real_idx].append([normalized_x, normalized_y])
                    del tmp
                    ###############################
                    tmp = bboxes[real_idx]
                    bboxes[real_idx] = []
        ...
        for i, phrase in enumerate(phrases):
            question = random.choice(BOX_POINT_QUESTIONS).format(class_name=phrase)
            if i == 0:
                question = self.begin_str + question
            conversation.append({'from': 'human', 'value': question})
            answer = random.choice(BOX_POINT_ANSWER_LIST).format(class_name=phrase,coords="{coords}",point_coords="{point_coords}")
    ...
    def prepare_data(self, index):
        ...
        if 'points' in data_dict:
            out_data_dict['points'] = data_dict['points']
        ...
        if data_dict.get('image', None) is not None:
            ...
            token_dict = self.get_inputid_labels(data_dict['conversations'], data_dict['bboxes'], data_dict['points'], image_token_str)
            out_data_dict.update(token_dict)
        else:
            token_dict = self.get_inputid_labels(data_dict['conversations'], data_dict['bboxes'], data_dict['points'], None)
            out_data_dict.update(token_dict)
    ...
    def get_inputid_labels(self, conversations, bboxes, points, image_token_str) -> dict:
        input = ''
        out_conversation = []
        # pdb.set_trace()
            ...
            if msg['from'] == 'human':
                ...
                if type(bbox[0]) is list:
                    ...
                    for i in range(num_boxes):
                        ...
                        point_str += f"{point[i][0]:.4f}, {point[i][1]:.4f}"
                        if i < num_boxes - 1:
                            coords_str += "<sep>"
                            point_str += "<sep>"
                else:
                    coords_str = f"{bbox[0]:.4f}, {bbox[1]:.4f}, {bbox[2]:.4f}, {bbox[3]:.4f}"
                    point_str = f"{point[0]:.4f}, {point[1]:.4f}"
                    
                msg_value = msg['value'].replace('{coords}', coords_str).replace('{point_coords}', point_str)
                # print(msg_value)
                out_conversation.append({
                    'input': input,
                    'output': msg_value.strip() + '<single_img>.'
                })
                input = ''
                bbox_idx += 1
            else:
