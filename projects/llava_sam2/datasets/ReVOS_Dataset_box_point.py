...
from .encode_fn import video_lisa_encode_fn, video_lisa_encode_fn_box_point
import json
import random
...
BOX_POINT_QUESTIONS = [
    "Detect and locate the {class_name} in this video with bounding boxes and key points.",
    "Track the {class_name} with boxes and mark some points on it.",
    "Identify and track the {class_name} in the video.",
    
    "Track the {class_name}.",
    "Can you identify and locate the {class_name} in this video?",
    
    "Identify the {class_name} in the video and mark its key locations.",
    "Where is the {class_name} in this video? Track and annotate it.",
    
    "Could you track and locate the {class_name}?",
    "Detect and identify the {class_name} in this video.",
    "Can you detect and track the {class_name}?"
]

BOX_POINT_ANSWER_LIST = [
    "Sure, the object locations are <box>{coords}</box> and the key points are <points>{point_coords}</points>.",
    "The object trajectories are <box>{coords}</box> with representative points <points>{point_coords}</points>.",
    "The object positions in this video are <box>{coords}</box> and marked points are <points>{point_coords}</points>.",
    "The tracking results are <box>{coords}</box> with key points <points>{point_coords}</points>.",
    "The object movements in this video are <box>{coords}</box> and the marked points are <points>{point_coords}</points>.",
    "Here are the object trajectories: <box>{coords}</box> and points: <points>{point_coords}</points>.",
    "The tracked positions are <box>{coords}</box> with key points <points>{point_coords}</points>.",
    "Sure, the object trajectories are <box>{coords}</box> and representative points are <points>{point_coords}</points>."
]

class VideoReVOSDataset_box_point(Dataset):
    ...
    def __init__(self,
        ...
        if offline_processed_text_folder is not None:
            ...
            vid2metaid, metas, mask_dict, box_dict = self.json_file_preprocess(expression_file, mask_file, box_file)
    ...
    def json_file_preprocess(self, expression_file, mask_file, box_file):
        ...
        return vid2metaid, metas, mask_dict, box_dict
    ...
    def decode_bbox_points(self, video_masks, video_bboxes, image_size):
        ori_height, ori_width = image_size
        all_expressions_areas = []
        all_expressions = []
        all_expressions_points = []
        ...
        for exp_id, exp_box in enumerate(video_bboxes):  
            ...
            for frame_idx in range(len_frames):
                ...
                for obj_idx in range(num_objects):
                    ...
                    if object_masks[obj_idx][frame_idx] is not None:
                        m = maskUtils.decode(object_masks[obj_idx][frame_idx])  
                        dist_transform = cv2.distanceTransform(m.squeeze(), cv2.DIST_L2, 5)
                        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(dist_transform)
                        normalized_x = max_loc[0] / ori_width
                        normalized_y = max_loc[1] / ori_height
                        ##########################
                        # 50%概率添加抖动
                        ...
                        #     pixel_x = int(jitter_normalized_x * ori_width)
                        #     pixel_y = int(jitter_normalized_y * ori_height)
                        #     if m.squeeze()[pixel_y, pixel_x] > 0: # 确保在mask内
                        #         normalized_x = jitter_normalized_x
                        #         normalized_y = jitter_normalized_y
                        valid_points_per_obj.append(f"{normalized_x:.4f},{normalized_y:.4f}")
                        ##########################
        
                bbox_per_frame[frame_idx] = frame_total_area
                
                if valid_coords_per_obj:
                    box_expr = f"<box>{('<sep>'.join(valid_coords_per_obj))}</box>"
                    points_expr = f"<points>{('<sep>'.join(valid_points_per_obj))}</points>"
                else:
                    box_expr = "<box><no_target></box>"
                    points_expr = "<points><no_target></points>"
                    
                box_expressions.append(box_expr)
                points_expressions.append(points_expr)

            all_expressions_areas.append(bbox_per_frame)
            all_expressions.append(box_expressions)
            all_expressions_points.append(points_expressions)
        
        return all_expressions_areas, all_expressions, all_expressions_points
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        for object_info in data_dict:
            ...
            for anno_id in anno_ids:
                ...
                frames_boxes = self.box_dict[anno_id]
                frames_masks_ = []
                frames_boxes_ = []
                for frame_idx in selected_frame_indexes:
                    ...
                    frames_boxes_.append(copy.deepcopy(frames_boxes[frame_idx]))
                obj_masks.append(frames_masks_)
                obj_boxes.append(frames_boxes_)
            video_masks.append(obj_masks)
            video_boxes.append(obj_boxes)
        ...
        ret = {'images': images, 'video_masks': video_masks, 'video_boxes': video_boxes, 'conversation': text_dict['conversation'],
    ...
    def prepare_text(self, n_frames, expressions, num_image_tokens=256, n_fast_images=50):
        ...
        for i, exp in enumerate(expressions):
            ...
            if '?' in exp:
                ...
                question_template = random.choice(BOX_POINT_QUESTIONS)
                questions.append(question_template.format(class_name=exp.lower()))
            answers.append(random.choice(BOX_POINT_ANSWER_LIST))
    ...
    def __getitem__(self, index):
        ...
        if data_dict.get('images', None) is not None:
            ...
            bboxes_areas, bboxes_expression, bboxes_points = self.decode_bbox_points(data_dict['video_masks'], data_dict['video_boxes'], image_size=(ori_height, ori_width))
            # if masks is None:
            ...
            data_dict['bboxes_points'] = bboxes_points
        else:
        ...
        result = video_lisa_encode_fn_box_point(data_dict, tokenizer=self.tokenizer, max_length=self.max_length)
        data_dict.update(result)
